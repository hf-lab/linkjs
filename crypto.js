...
function exception(){
	...
	setting.protected_links = setting.protected_links;
	exception = setting.protected_links.split(",");
	return exception;
}

function spesific_links(){var spesific_links=new Array();setting.spesific_links=setting.spesific_links;spesific_links=setting.spesific_links.split(",");return spesific_links;}
...
// new
// old
if (!setting.protected_links) {
	setting.protected_links = window.location.href;
} else {
	setting.protected_links += "," + window.location.href;
}
var exception = exception();
var spesific_links = spesific_links();
var links =new Array();		
function showurl(datajson){

	var exceptionlength = exception.length;
	var spesific_linkslength = spesific_links.length;
	var checklink = "";
	var checkexception = "";
	var linktag = document.getElementsByTagName("a");

	if (setting.protected == false && setting.spesific == false) {
	...
	for (var i = 0; i < linktag.length; i++) {
		...
		checkprotected_links = true;
		checkspesific_links = false;
		no = 0;
		if (setting.protected) {
			checkprotected_links = false;
			while (checkprotected_links == false && no < exceptionlength) {
		...
		if (setting.spesific) {
			checkspesific_links = false;
			while (checkspesific_links == false && no < spesific_linkslength) {
				checklink = extractDomain(linktag[i].href);
				checkexception = extractDomain(spesific_links[no]);
				if (checklink.match(checkexception)) {
					checkspesific_links = true;
				}
				no++;
			}
		}
		if (checkprotected_links == false || checkspesific_links == true) {
